...
-------------------------------
  ...
  Using Arduino UNO for Coordinator :  
...
//      { nss.println("Coordinator Started! Welcom to XBee ZigBee Network!"); }
//    }
  ...
  if(coordinator.readPacket(5000)){
    ...
    if(coordinator.getResponse().getApiId() == AT_COMMAND_RESPONSE){
      ...
      if(cAtResponse.isOk()){
        ...
        nss.println(cAtResponse.getStatus(), HEX);
      }
    }
    else{
      // ApiTd != AT_COMMAND_RESPONSE
      nss.print("Expected AT response but got ");
      nss.print(coordinator.getResponse().getApiId(), HEX);
    }
  }
  else{
    // At command failed 
    if(coordinator.getResponse().isError()){
      ...
      nss.println(coordinator.getResponse().getErrorCode());
    }
    else{
